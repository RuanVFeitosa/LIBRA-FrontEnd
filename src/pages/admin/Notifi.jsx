...
const urgencyIcons = {
  urgent: <AiOutlineExclamationCircle className="icon" style={{backgroundColor:'#b92922'}} />,
  moderate: <AiOutlineExclamationCircle className="icon" style={{backgroundColor:'#de8b1c'}} />,
  low: <AiOutlineExclamationCircle className="icon" style={{backgroundColor:'#3bab64'}} />,
};
      ...
      <container className="background">
        <div className="container-pai">
          <div className="navBar">
            <h2>
              Bom dia, <b>Samantha!</b>
            </h2>
            <p>
              Voce tem 1{" "}
              <a href="#" style={{ color: "#00bf15" }}>
                notificação
              </a>
            </p>
            <IoMdExit />
          </div>
          <hr style={{ width: "101%", margin: "0px auto", }} />
          <div id="titulo">
            <h1>
              Notificações
              <hr style={{ width: "101%"}} />
            ...
            {notifications.map((notification, index) => (
              <div key={index} className="notif-container">
                ...
                {notification.urgency && (
                  ...
                  {notification.author && (
                    <p>Autor(a): {notification.author}</p>
                  )}
                  {notification.requestedBy && (
                    <p>Solicitado por: {notification.requestedBy}</p>
                  )}
                  {notification.event && <p>Evento: {notification.event}</p>}
                  {notification.specialNeed && (
                    <p>Necessidade especial: {notification.specialNeed}</p>
                  )}
          ...
          <div className="nivel">
            <p>
              {" "}
              <AiOutlineExclamationCircle size={40} id="ExU" /> Urgente (Prazo expira em
              menos de 15 dias)
            </p>
            <p>
              {" "}
              <AiOutlineExclamationCircle size={40} id="ExA" /> Atenção moderada (Prazo
              expira em um mês)
            </p>
            <p>
              <AiOutlineExclamationCircle size={40} id="ExS" /> Sem urgência (prazo expira
              em um mês e meio ou mais)
            </p>
          </div>
        </div>
      </container>
